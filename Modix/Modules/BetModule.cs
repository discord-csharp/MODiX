...
namespace Modix.Modules
{
    ...
    public class BetModule : ModuleBase
    {
        ...
        [Command("Stage"),
         Summary("Sets the stage for a new betting game. Players can join during this time with !bet join")]
        public async Task StageAsync()
        {
            if (_state == BettingState.Idle)
            {
                ...
                await Context.Channel.SendMessageAsync(
                    $"{Context.User.Mention} has set the stage for a new betting game. `!bet join` to participate in this session. `!bet start` to begin!");
            ...
            {
                await Context.Channel.SendMessageAsync(
                    $"MODiX is currently setting the stage for a bet. There is only `{StageDurationInSeconds - _stageCounter}` seconds left to join. `!bet join` to participate in this session");
            }
            else if (_state == BettingState.Stage && Context.User.Id == _sessionOwner.Id)
            {
                await Context.Channel.SendMessageAsync(
                    $"You're the owner of the current session, {Context.User.Mention}. Type `!bet start` to begin the game!");
            }
            else
            {
                await Context.Channel.SendMessageAsync(
                    "MODiX is currently in a bet. Please wait for the current game to end");
            }
        }
        ...
        public async Task StartAsync()
        {
            if (_state == BettingState.Stage && Context.User.Id == _sessionOwner.Id)
            {
                ...
                await Context.Channel.SendMessageAsync(
                    "Ok everyone, The stage is set! `!bet roll` to place your bets!");
            ...
            {
                await Context.Channel.SendMessageAsync(
                    "There is no bet currently in progress. `!bet stage` to start a new session");
            }
        }
        ...
        {
            if (_state == BettingState.Stage && Context.User.Id != _sessionOwner.Id &&
                !_joinedUsers.Any(u => u.Id == Context.User.Id))
            {
        ...
        {
            if (_state == BettingState.Rolling && _joinedUsers.Any(u => u.Id == Context.User.Id) &&
                !_rolls.Any(r => r.Key.Id == Context.User.Id))
            {
        ...
        private async void StageTimer_Tick(object state)
        {
            ...
            {
                await Context.Channel.SendMessageAsync(
                    $"Not enough players joined {Context.User.Mention}'s session. Start another game with `!bet stage`");
                ResetStageTimer();
            }
        ...
        private async void RollTimer_Tick(object state)
        {
            ...
            {
                await Context.Channel.SendMessageAsync(
                    $"Well this is awkward... no one rolled. Really {_sessionOwner.Mention}, you didn't even roll in your own game?");
                _state = BettingState.Idle;
                ResetRollTimer();
                return;
            }

            if (_rollingCounter >= RollingDurationInSeconds || _state == BettingState.Results)
            {
                // Show the results!
                ...
                var builder = new EmbedBuilder()
                    ...
                    .WithFooter(
                        new EmbedFooterBuilder().WithText(
                            $"Thank you {_sessionOwner.Username} for hosting this game."));
            ...
            if (RollingDurationInSeconds - _rollingCounter <= 5)
